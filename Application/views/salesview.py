...
def shopping_cart():
    ...
    if request.method == 'POST':
        ...
        if product_id > len(Products.products):
            ...
            cartID = len(currentShoppingCart)
            ...
            currentShoppingCart["total_price"] += quantity*Products.products[product_id]["unit_price"]
            currentShoppingCart["total_items_in_cart"] += quantity         
            ...
            return jsonify ({"Response":response, "Items in Cart":currentShoppingCart}), 200

    if len(currentShoppingCart) > 0:        
        return jsonify ({"Items in Cart": currentShoppingCart}), 200  

    response = "Cart is empty!"
    return jsonify ({"Response":response, "Items in Cart": currentShoppingCart})  

@blue_print.route('/v1/sales/create_record')
def createSalesRecord():
    global user
    global currentShoppingCart

    
    time = datetime.now()            
    details = currentShoppingCart            
    total_price = currentShoppingCart["total_price"]
    items= currentShoppingCart["total_items_in_cart"]
    dict = {'user':user,
            'items': items,
